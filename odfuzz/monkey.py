...
The objects created by PyOData library are altered in order to set up a proper interface for HTTP request generation.

When the fuzzer starts generating requests, it goes through this common routine frequently:
...
In this module, the methods 'operators()' and 'generate()' are dynamically patched to the existing
properties' objects. Therefore, it is possible to call `property.generate()` , while
